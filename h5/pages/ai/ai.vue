<template>
  <view class="page">
    ...
    <view class="prompt-card">
      <u-textarea v-model="prompt" placeholder="例如：想找一本轻松但有深度的历史书" border="none" height="96" />
      <u-button color="#606C38" text="生成推荐" @click="toast" />
    </view>

    <view class="quick-row">
      <u-tag text="通勤阅读" shape="circle" bg-color="#606C38" border-color="#606C38" color="#FFFDF7" />
      <u-tag text="亲子共读" shape="circle" plain color="#606C38" border-color="rgba(96,108,56,.18)" />
      <u-tag text="管理入门" shape="circle" plain color="#606C38" border-color="rgba(96,108,56,.18)" />
    </view>

    <u-cell-group :border="false" custom-style="border-radius: 28rpx; overflow: hidden; background: #FFFDF7;">
...
.page {
  min-height: 100vh;
  padding: calc(24rpx + env(safe-area-inset-top)) 28rpx calc(132rpx + env(safe-area-inset-bottom));
  background: #f8f4ea;
  color: #2c2416;
}
