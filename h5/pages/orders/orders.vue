<template>
    <view class="page"><u-navbar title="" :bg-color="theme.page" :border="false" :placeholder="true"
        ...
        <view class="topline">
            ...
            <view><text class="title">我的订单</text><text class="sub">待付款、待发货、待收货、售后与退款</text></view>
        </view><scroll-view class="tabs" scroll-x show-scrollbar="false">
            <view class="tabs-track"><view v-for="tab in tabs" :key="tab.key" class="tab" :class="{ active: status === tab.key }"
                @tap="select(tab.key)">{{ tab.label }}</view></view>
        </scroll-view>
        <view v-if="loading" class="loading"><u-loading-icon color="#606C38" /></view>
        <view v-else-if="loadError" class="load-error"><u-icon name="reload" color="#C66B3D"
                size="28" /><text>订单加载失败</text><u-button size="mini" color="#C66B3D" text="重新加载" @click="load" /></view>
        ...
        <view v-else class="order-list">
            <view v-for="order in orders" :key="order.id" class="order-card" @tap="goDetail(order.id)">
                <view class="order-head"><text>{{ order.orderNo }}</text><text class="status">{{
...
<script lang="ts" setup>
    ...
    const theme = { page: '#F8F4EA' }; const orders = ref<Order[]>([]); const loading = ref(true); const loadError = ref(false); const status = ref<OrderStatus | ''>('');
    let close = () => undefined;
    let loadingTask: Promise<void> | null = null
    const tabs = [{ key: '', label: '全部' }, { key: 'PENDING', label: '待付款' }, { key: 'PAID', label: '待发货' }, { key: 'SHIPPED', label: '待收货' }, { key: 'AFTER_SALE', label: '售后中' }, { key: 'REFUNDING', label: '退款中' }] as { key: OrderStatus | ''; label: string }[]
    onLoad((options) => {
        ...
        status.value = validStatuses.includes(queryStatus as OrderStatus) ? queryStatus as OrderStatus : ''
        load(); close = connectOrderRealtime(() => load())
    ...
    async function load() {
        ...
        try { await loadingTask } finally { loadingTask = null }
    }
    ...
    async function loadWithRetry() {
        ...
        try {
            ...
            for (let attempt = 0; attempt < 2; attempt += 1) {
                try { orders.value = (await getOrdersApi(status.value || undefined)).data; return } catch (error) {
    ...
    function select(value: OrderStatus | '') { status.value = value; load() }
    function goDetail(id: number) { uni.navigateTo({ url: `/pages/order-detail/order-detail?id=${id}` }) }
    function goPayment(id: number) { uni.navigateTo({ url: `/pages/payment/payment?id=${id}` }) }
    // 退款和售后共用申请页，但会携带不同类型以更新各自订单状态。
    function goRefund(id: number) { uni.navigateTo({ url: `/pages/refund/refund?id=${id}&type=refund` }) }
    function goAfterSale(id: number) { uni.navigateTo({ url: `/pages/refund/refund?id=${id}&type=after-sale` }) }
    function remindShipment() { uni.showToast({ title: '已提醒商家发货', icon: 'none' }) }
    async function complete(id: number) {
        try { await completeOrderApi(id); uni.showToast({ title: '已确认收货', icon: 'none' }); await load() } catch { /* 请求错误由统一请求层提示 */ }
    }
    async function remove(id: number) {
        try { await deleteOrderApi(id); uni.showToast({ title: '订单已删除', icon: 'none' }); await load() } catch { /* 请求错误由统一请求层提示 */ }
    }
    function back() { uni.navigateBack({ delta: 1 }) } function statusText(value: OrderStatus) { return ({ PENDING: '待付款', PAID: '待发货', SHIPPED: '待收货', COMPLETED: '已完成', CANCELLED: '已取消', AFTER_SALE: '售后中', REFUNDING: '退款中', REFUNDED: '已退款', REJECTED: '退款驳回' } as Record<OrderStatus, string>)[value] }
    // 订单卡片只展示第一件商品，数量和价格分别使用整单数量与订单总价。
    function totalItemCount(order: Order) { return order.items.reduce((sum, item) => sum + item.quantity, 0) }
...
<style lang="scss" scoped>
    ...
    .tab {
        ...
        font-size: 23rpx;
        font-weight: 700;
        box-shadow: 0 4rpx 12rpx rgba(44, 36, 22, .04);
    }
    ...
    .order-actions {
        ...
        margin-top: 18rpx;
        padding-top: 16rpx;
        border-top: 1rpx solid rgba(159, 147, 129, .16)
    }
