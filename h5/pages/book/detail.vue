  ...
  <view class="page" :style="pageStyle">
    ...
    <view class="detail-card">
      ...
      <text class="book-author"
        >{{ book.author || '未知作者' }} · {{ book.language || '中文' }}</text
      >

      <view class="meta-row">
        <view class="meta-box"
          ><text class="meta-value">{{ book.rating || '-' }}</text
          ><text class="meta-label">评分</text></view
        >
        <view class="meta-box"
          ><text class="meta-value">{{ book.salesCount }}</text
          ><text class="meta-label">销量</text></view
        >
        <view class="meta-box"
          ><text class="meta-value">{{ book.stock }}</text
          ><text class="meta-label">库存</text></view
        >
      </view>
      ...
      <view class="section">
        <text class="paragraph">{{
          book.description || '后台图书管理可维护简介、封面、价格、库存、分类与标签。'
        }}</text>
      </view>
    </view>
    ...
    <view class="bottom-bar">
      ...
      <u-button
        ...
        text="加入购物车"
        :loading="adding"
        :disabled="book.status !== 1 || book.stock < 1"
        @click="addToCart"
      />
    </view>
  </view>
...
const pageStyle = computed(() =>
  statusBarHeight.value ? { paddingTop: `calc(${statusBarHeight.value}px + 116rpx)` } : {},
)
...
async function toggleFavorite() {
  if (!uni.getStorageSync('token')) {
    uni.navigateTo({
      url: `/pages/login/login?redirect=${encodeURIComponent(`/pages/book/detail?id=${book.value.id}`)}`,
    })
    return
  }
...
function createBook(
  ...
  return {
    ...
    price,
    originalPrice: null,
    description: '从真实业务表读取图书基础信息、分类、标签、价格、库存和阅读建议。',
    language: '中文',
    stock,
    salesCount: 1280,
    viewCount: 0,
...
.detail-cover {
  ...
  background: #c66b3d;
  box-shadow:
    inset -28rpx 0 rgba(44, 36, 22, 0.1),
    0 18rpx 46rpx rgba(44, 36, 22, 0.16);
}
...
.bottom-bar {
  ...
  background: #fffdf7;
  box-shadow: 0 -10rpx 30rpx rgba(44, 36, 22, 0.08);
}
