  ...
  <view class="page">
    ...
    <view v-else class="cart-list">
      <view v-for="item in cart.items" :key="item.id" class="cart-item">
        <view class="check-mark" :class="{ checked: editing ? editSelectedIds.has(item.id) : item.selected }"
          @tap="editing ? toggleEditSelected(item.id) : toggleSelected(item)">
          <u-icon v-if="editing ? editSelectedIds.has(item.id) : item.selected" name="checkmark" color="#7A7A7A"
            size="16" />
        </view>
        ...
        <view class="item-main">
          ...
          <text v-if="!isAvailable(item)" class="warning">{{ item.book.stock ? '图书已下架' : '库存不足' }}</text>
          <view class="item-foot">
            <text class="price">¥{{ formatPrice(item.book.price) }}</text>
            <view class="quantity-box">
              <view class="quantity-button" :class="{ disabled: item.quantity <= 1 }" @tap="changeQuantityBy(item, -1)">
                −
              </view>
              <text class="quantity-value">{{ item.quantity }}</text>
              <view class="quantity-button" :class="{ disabled: item.quantity >= item.book.stock }"
                @tap="changeQuantityBy(item, 1)">+</view>
            </view>
          </view>
        </view>
    ...
    <view v-if="cart.items.length" class="summary">
      ...
      <template v-else>
        ...
        <u-button class="checkout" color="#C66B3D" :disabled="!cart.selectedCount" text="去结算" @click="checkout" />
      </template>
    </view>
...
<script lang="ts" setup>
  ...
  import { addFavoritesApi, batchDeleteCartItemsApi, getCartApi, removeCartItemApi, updateAllCartSelectedApi, updateCartQuantityApi, updateCartSelectedApi } from '../../services/bookstore'
  ...
  const allEditSelected = computed(() => cart.value.items.length > 0 && cart.value.items.every((item) => editSelectedIds.value.has(item.id)))
  ...
  async function loadCart() {
    loading.value = true
    try { cart.value = (await getCartApi()).data } finally { loading.value = false }
  }

  async function toggleSelected(item: CartItem) {
    try { cart.value = (await updateCartSelectedApi(item.id, !item.selected)).data } catch { await loadCart() }
  }
  ...
  async function toggleAll() {
    ...
    try { cart.value = (await updateAllCartSelectedApi(selected)).data } catch { await loadCart() }
  }

  async function changeQuantity(item: CartItem) {
    try { cart.value = (await updateCartQuantityApi(item.id, item.quantity)).data } catch { await loadCart() }
  }
  ...
  function remove(id: number) {
    uni.showModal({
      ...
      success: async ({ confirm }) => {
        if (!confirm) return
        try { cart.value = (await removeCartItemApi(id)).data } catch { await loadCart() }
      },
    })
  }
  ...
  function removeSelected() {
    ...
    uni.showModal({
      title: '删除商品', content: `确定删除选中的 ${ids.length} 件商品吗？`, confirmText: '确定', cancelText: '取消',
      confirmColor: '#E53935', cancelColor: '#2F80ED',
      success: async ({ confirm }) => {
        if (!confirm) return
        try { cart.value = (await batchDeleteCartItemsApi(ids)).data; editSelectedIds.value = new Set() } catch { await loadCart() }
      },
    })
  }

  async function favoriteSelected() {
    const bookIds = cart.value.items.filter((item) => editSelectedIds.value.has(item.id)).map((item) => item.book.id)
    if (!bookIds.length) return uni.showToast({ title: '请先选择商品', icon: 'none' })
    try { await addFavoritesApi(bookIds); uni.showToast({ title: '已加入收藏', icon: 'success' }) } catch { /* 请求层已提示错误 */ }
  }
  ...
  function shareSelected() {
    ...
    if (typeof (uni as any).share === 'function') {
      ; (uni as any).share({ provider: 'system', type: 0, title: `我想和你分享：${title}`, summary: title, success: () => uni.showToast({ title: '分享成功', icon: 'success' }) })
    } else {
  ...
  function checkout() { uni.navigateTo({ url: '/pages/order-confirm/order-confirm' }) }
  function isAvailable(item: CartItem) { return item.book.status === 1 && item.book.stock >= item.quantity }
  function coverStyle(url: string | null) { return url ? { backgroundImage: `url(${normalizeAssetUrl(url)})` } : {} }
  function formatPrice(value: number | string | null) { return Number(value ?? 0).toFixed(2) }
</script>
...
<style lang="scss" scoped>
  ...
  .summary {
    ...
    padding: 18rpx 28rpx;
    background: #fffdf7;
    box-shadow: 0 -8rpx 24rpx rgba(44, 36, 22, .08);
  }
