  ...
  <view class="page"
    ...
    <view class="topline">
      ...
      <view
        ...
        ><text class="sub">{{ order?.orderNo || '加载中' }}</text></view
      >
    </view>
    <view v-if="order" class="content">
      <view class="state"
        ><text class="state-title">{{ statusText(order.status) }}</text
        ><text class="state-sub">订单状态实时更新</text
        ><text class="state-time">下单时间：{{ formatDate(order.createdAt) }}</text></view
      >
      <view class="panel address"
        ><u-icon name="map" color="#C66B3D" size="25" />
        <view
          ><text>{{ order.addressSnapshot.receiverName || '收货信息' }}</text
          ><text>{{ order.addressSnapshot.receiverPhone || '' }}</text
          ><text>{{ order.addressSnapshot.detail || '暂无地址' }}</text></view
        >
      </view>
      <view class="panel"
        ...
        <view v-for="item in order.items" :key="item.id" class="line">
          ...
          <view class="line-copy"
            ><text>{{ item.book.title || '图书商品' }}</text
            ><text>{{ item.quantity }} 件</text> </view
          ><text class="amount">¥{{ (item.price * item.quantity).toFixed(2) }}</text>
        </view>
      </view>
      <view class="panel summary"
        ><text>订单金额</text><text class="total">¥{{ order.totalAmount.toFixed(2) }}</text>
      </view>
      <view class="actions"
        ><u-button
          v-if="order.status === 'PAID' || order.status === 'AFTER_SALE'"
          color="#C66B3D"
          text="申请退款"
          @click="refund"
        />
...
onLoad(async (options) => {
  ...
  if (id) {
    await loadOrder(id)
    close = connectOrderRealtime((data) => {
      const message = data as { id?: number; deleted?: boolean }
      if (message.id === id && message.deleted) {
        order.value = undefined
        uni.redirectTo({ url: '/pages/orders/orders' })
        return
      }
      if (message.id === id) loadOrder(id)
    })
  }
})
...
async function loadOrder(id: number) {
  order.value = (await getOrderApi(id)).data
}
async function pay() {
  if (order.value) {
...
function refund() {
  if (order.value) uni.navigateTo({ url: `/pages/refund/refund?id=${order.value.id}&type=refund` })
}
function afterSale() {
  if (order.value)
    uni.navigateTo({ url: `/pages/refund/refund?id=${order.value.id}&type=after-sale` })
}
...
function statusText(v: OrderStatus) {
  return (
    {
      PENDING: '待付款',
      PAID: '待发货',
