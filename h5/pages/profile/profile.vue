<template>
  <view class="page">
    ...
    <view class="page-topbar"
      ...
      ><text class="page-title">个人资料</text></view
    >
    <view class="profile-card">
      <view class="avatar"><u-icon name="account" color="#FFFDF7" size="30" /></view>
      <view class="profile-copy"
        ><text class="eyebrow">账号资料</text
        ><text class="name">{{ form.nickname || form.username || '未登录' }}</text
        ><text class="account">账号：{{ form.username || '暂无账号' }}</text></view
      >
    </view>
    ...
    <view class="form-panel">
      <view class="field"
        ...
        ><view class="field-value readonly">{{ form.username || '暂无账号' }}</view></view
      >
      <view class="field"
    ...
    <view class="hint"
      ><u-icon name="lock" color="#A0927F" size="15" /><text>账号安全信息由系统统一保护</text></view
    >
  </view>
</template>
...
onShow(async () => {
  ...
  try {
    Object.assign(form, (await getProfileApi()).data)
  } catch {
    /* 请求层统一提示错误 */
  }
})
...
async function save() {
  if (!uni.getStorageSync('token'))
    return uni.navigateTo({ url: '/pages/login/login?redirect=/pages/profile/profile' })
  saving.value = true
  try {
