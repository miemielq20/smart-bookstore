...
export const getCartApi = () => request<CartResponse>({ url: '/cart' })
...
export const getFavoritesApi = () => request<FavoriteListResponse>({ url: '/favorites' })

export const createOrderApi = (data: { address: Record<string, string>; remark?: string }) =>
  request<OrderResponse>({ url: '/orders', method: 'POST', data })
export const getOrdersApi = (status?: OrderStatus) =>
...
export const payOrderApi = (id: number) =>
  request<OrderResponse>({ url: `/orders/${id}/pay`, method: 'POST' })
export const completeOrderApi = (id: number) =>
  request<OrderResponse>({ url: `/orders/${id}/complete`, method: 'POST' })
export const cancelOrderApi = (id: number) =>
  request<OrderResponse>({ url: `/orders/${id}/cancel`, method: 'POST' })
// 删除待付款或待发货订单，并由后端事务清理订单关联数据。
export const deleteOrderApi = (id: number) =>
  request<{ id: number; deleted: boolean }>({ url: `/orders/${id}`, method: 'DELETE' })
export const refundOrderApi = (id: number, reason: string) =>
  request<OrderResponse>({ url: `/orders/${id}/refund`, method: 'POST', data: { reason } })
// 售后申请单独调用接口，避免与退款审核订单混用同一状态。
export const afterSaleOrderApi = (id: number, reason: string) =>
  request<OrderResponse>({ url: `/orders/${id}/after-sale`, method: 'POST', data: { reason } })
export const getAddressesApi = () => request<AddressResponse>({ url: '/addresses' })
export const createAddressApi = (
  data: Omit<Address, 'id' | 'isDefault'> & { isDefault?: number },
) => request<AddressResponse>({ url: '/addresses', method: 'POST', data })
export const updateAddressApi = (
  id: number,
  data: Omit<Address, 'id' | 'isDefault'> & { isDefault?: number },
) => request<AddressResponse>({ url: `/addresses/${id}`, method: 'PUT', data })
export const setDefaultAddressApi = (id: number) =>
  request<AddressResponse>({ url: `/addresses/${id}/default`, method: 'POST' })
export const removeAddressApi = (id: number) =>
  request<AddressResponse>({ url: `/addresses/${id}`, method: 'DELETE' })
