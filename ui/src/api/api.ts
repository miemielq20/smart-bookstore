...
import type {
  ...
  GetCodeApiResponse,
  MenusApiResponse,
  getMenuTreeApiResponse,
  getGroupsApiResponse,
  getGroupsMenuApiResponse,
  ApiResponse,
  BookListApiResponse,
  BookDetailApiResponse,
  BookCategoriesApiResponse,
...
import type {
  ...
  AdminOrderStatsApiResponse,
  AdminRefundListApiResponse,
} from '@/type/api.response'
...
// 获取验证码
export const getCodeApi = (): Promise<GetCodeApiResponse> => request.get('/getCode')

/* GET /api/auth/menus */
export const getMenusApi = (): Promise<MenusApiResponse> => request.get('/auth/menus')

// 加载权限组信息
export const getGroupsApi = (): Promise<getGroupsApiResponse> => request.get('/permission-groups')
...
): Promise<CategoryDetailApiResponse> => request.patch(`/categories/${id}`, data)

export const updateCategoryStatusApi = (
  id: number,
  status: number,
): Promise<CategoryDetailApiResponse> => request.patch(`/categories/${id}/status`, { status })
...
): Promise<BannerDetailApiResponse> => request.patch(`/banners/${id}`, data)

// 切换 Banner 启禁用状态
export const updateBannerStatusApi = (
  id: number,
  status: number,
): Promise<BannerDetailApiResponse> => request.patch(`/banners/${id}/status`, { status })
...
): Promise<AdminOrderListApiResponse> => request.get('/admin/orders', { params })
export const getAdminOrderApi = (id: number): Promise<AdminOrderDetailApiResponse> =>
  request.get(`/admin/orders/${id}`)
export const getAdminOrderStatsApi = (): Promise<AdminOrderStatsApiResponse> =>
  request.get('/admin/orders/stats')
export const shipAdminOrderApi = (
  id: number,
  trackingNo: string,
): Promise<AdminOrderDetailApiResponse> => request.put(`/admin/orders/${id}/ship`, { trackingNo })
export const updateAdminOrderRemarkApi = (
  id: number,
  remark: string,
): Promise<AdminOrderDetailApiResponse> => request.put(`/admin/orders/${id}/remark`, { remark })
export const cancelAdminOrderApi = (id: number): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/orders/${id}/cancel`)
// 管理员确认售后后，将订单转入退款审核状态。
export const approveAdminAfterSaleApi = (id: number): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/orders/${id}/after-sale/approve`)
// 售后拒绝和重新发货接口。
export const rejectAdminAfterSaleApi = (id: number): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/orders/${id}/after-sale/reject`)
export const reshipAdminAfterSaleApi = (
  id: number,
  trackingNo: string,
): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/orders/${id}/after-sale/reship`, { trackingNo })
export const getAdminRefundsApi = (
  params: AdminRefundQueryParams,
): Promise<AdminRefundListApiResponse> => request.get('/admin/refunds', { params })
export const approveAdminRefundApi = (
  id: number,
  handlerNote = '',
): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/refunds/${id}/approve`, { handlerNote })
export const rejectAdminRefundApi = (
  id: number,
  handlerNote: string,
): Promise<AdminOrderDetailApiResponse> =>
  request.put(`/admin/refunds/${id}/reject`, { handlerNote })
