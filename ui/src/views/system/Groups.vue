<template>
  <div>
    ...
    <div class="card-panel" style="padding:0;">
      <el-table :data="groups" class="groups-table" header-row-class-name="table-header" :ref="groupsTable">
    ...
    <el-dialog v-model="createVisible" title="新增权限组" width="560px" destroy-on-close>
      <el-form :model="createForm" label-width="80px">
        ...
        <el-form-item label="菜单权限">
          <el-tree ref="createTreeRef" :default-expanded-keys="allParentIds" :data="menuTree" show-checkbox
            node-key="id" :default-checked-keys="createForm.menuIds" :props="{ label: 'name', children: 'children' }" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="assignVisible" :title="`权限分配 · ${assignRow?.name || ''}`" width="560px" destroy-on-close>
      <el-tree ref="assignTreeRef" :default-expanded-keys="allParentIds" :data="menuTree" show-checkbox node-key="id"
        :default-checked-keys="assignMenuIds" :props="{ label: 'name', children: 'children' }" />
      <template #footer>
...
<script setup lang="ts">
  ...
  import { getGroupsApi, getMenuTreeApi, updateGroupsApi,addGroupsApi,getGroupsMenuApi} from "@/api/api"
  ...
  async function openAssignDialog(row: GroupRow) {
    assignRow.value = row
    assignMenuIds.value = []
    getGroupsMenuApi(row.id).then((res:any)=>{
