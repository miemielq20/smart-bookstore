...
export class FavoriteService {
  ...
  async list(userId: number) {
    ...
    const books = await this.prisma.books.findMany({
      where: { id: { in: rows.map((row) => row.bookId) }, deletedAt: null },
    })
  ...
  async add(userId: number, bookId: number) {
    ...
    const book = await this.prisma.books.findFirst({
      where: { id: BigInt(bookId), deletedAt: null },
    })
  ...
  async remove(userId: number, bookId: number) {
    await this.prisma.favorites.deleteMany({
      where: { userId: BigInt(userId), bookId: BigInt(bookId) },
    })
    return { success: true }
  }
}
