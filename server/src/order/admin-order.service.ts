...
export class AdminOrderService {
  ...
  async list(query: Record<string, string | undefined>) {
    ...
    const rows = await Promise.all(
      orders.map(async (order) => {
        ...
        const itemCount = await this.prisma.orderItems.aggregate({
    ...
    const filtered = keyword
      ? rows.filter((row) =>
          [row.orderNo, row.receiverName, row.receiverPhone, row.userNickname, row.userPhone].some(
            (value) =>
              String(value || '')
                .toLowerCase()
                .includes(keyword),
  ...
  async detail(id: number) {
    ...
    const [items, payments, refunds, users] = await Promise.all([
      ...
      this.prisma.refundRecords.findMany({
        where: { orderId: order.id },
        orderBy: { createdAt: 'desc' },
      }),
  ...
  async ship(id: number, trackingNo: string) {
    ...
    const updated = await this.prisma.orders.update({
      where: { id: order.id },
      data: { status: 'SHIPPED', trackingNo: trackingNo.trim(), shippedAt: new Date() },
    })
  ...
  async remark(id: number, remark: string) {
    const order = await this.prisma.orders
      .update({ where: { id: BigInt(id) }, data: { remark: remark.trim() || null } })
      .catch(() => {
  ...
  async stats() {
    ...
    return {
      ...
      afterSaleCount: counts.AFTER_SALE || 0,
      refundingCount: counts.REFUNDING || 0,
      todayCount: todayOrders.length,
      todayAmount: todayOrders.reduce((sum, item) => sum + Number(item.totalAmount), 0),
    }
  }
  ...
  async refunds(query: Record<string, string | undefined>) {
    ...
    const [total, records] = await Promise.all([
      this.prisma.refundRecords.count({ where }),
      this.prisma.refundRecords.findMany({
        where,
        orderBy: { createdAt: 'desc' },
    ...
    const orderMap = new Map(
      ...
      ).map((order) => [order.id.toString(), order]),
    )
    const users = await this.userMap(Array.from(orderMap.values()).map((order) => order.userId))
    return {
      list: records.map((record) => {
        const order = orderMap.get(record.orderId.toString())
        const user = order ? users.get(order.userId.toString()) : undefined
        return {
          id: Number(record.id),
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.refundRecords.update({
        ...
        data: {
          ...
          handlerId: BigInt(handlerId),
          handlerNote: handlerNote.trim() || null,
          handledAt: new Date(),
        },
      })
      if (approve) {
  ...
  private async userMap(ids: bigint[]) {
    const users = await this.prisma.userAccounts.findMany({
      where: { id: { in: ids } },
      select: { id: true, username: true, nickname: true, phone: true },
    })
    return new Map(users.map((user) => [user.id.toString(), user]))
  }
  private serialize(
    order: any,
    extra: { user?: any; address: Record<string, string>; itemCount: number },
  ) {
    return {
      id: Number(order.id),
      orderNo: order.orderNo,
      userId: Number(order.userId),
      userNickname: extra.user?.nickname || extra.user?.username || '-',
      userPhone: extra.user?.phone || '',
      receiverName: extra.address.receiverName || '',
      receiverPhone: extra.address.receiverPhone || '',
  ...
  private async publish(order: any) {
    const items = await this.prisma.orderItems.findMany({ where: { orderId: order.id }, take: 1 })
    this.realtime.publish(Number(order.userId), {
      id: Number(order.id),
      orderNo: order.orderNo,
      totalAmount: Number(order.totalAmount),
      status: order.status,
