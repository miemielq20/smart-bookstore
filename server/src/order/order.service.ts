...
export class OrderService {
  ...
  async create(userId: number, dto: { address?: Record<string, unknown>; remark?: string }) {
    ...
    const books = await this.prisma.books.findMany({ where: { id: { in: cart.map(item => item.bookId) }, deletedAt: null } })
    const bookMap = new Map(books.map(book => [book.id.toString(), book]))
    const rows = cart.map(item => ({ item, book: bookMap.get(item.bookId.toString()) })).filter(row => row.book)
    if (rows.length !== cart.length) throw new BadRequestException('部分图书已下架，请刷新购物车')
    for (const { item, book } of rows) {
      if (book!.status !== 1 || book!.stock < item.quantity) throw new BadRequestException(`${book!.title}库存不足或已下架`)
    }
    ...
    const order = await this.prisma.$transaction(async tx => {
      ...
      const created = await tx.orders.create({ data: { userId: user, orderNo, totalAmount: amount.toFixed(2), addressSnapshot, remark: dto.remark, expireAt: new Date(Date.now() + 30 * 60 * 1000) } })
      for (const { item, book } of rows) {
        const snapshot = { id: Number(book!.id), title: book!.title, author: book!.author, coverUrl: book!.coverUrl, price: Number(book!.price), originalPrice: book!.originalPrice === null ? null : Number(book!.originalPrice) }
        await tx.orderItems.create({ data: { orderId: created.id, bookId: book!.id, bookSnapshot: snapshot, price: book!.price, quantity: item.quantity } })
        await tx.books.update({ where: { id: book!.id }, data: { stock: { decrement: item.quantity }, salesCount: { increment: item.quantity } } })
  ...
  async list(userId: number, status?: OrderStatus) {
    ...
    const orders = await this.prisma.orders.findMany({ where: { userId: BigInt(userId), ...(status ? { status: status as any } : {}) }, orderBy: { createdAt: 'desc' } })
    // 订单列表需要展示完整商品和订单总价，不能只返回第一件商品。
    return Promise.all(orders.map(order => this.format(order)))
  }

  async detail(userId: number, id: number) {
    const order = await this.prisma.orders.findFirst({ where: { id: BigInt(id), userId: BigInt(userId) } })
  ...
  async pay(userId: number, id: number) {
    ...
    const result = await this.prisma.$transaction(async tx => {
      ...
      await tx.paymentRecords.create({ data: { orderId: order.id, paymentNo: `PAY${Date.now()}${Math.random().toString(36).slice(2, 8)}`, amount: order.totalAmount, status: 'SUCCESS', paidAt: now } })
      return tx.orders.findUniqueOrThrow({ where: { id: order.id } })
    })
    const data = await this.format(result); this.realtime.publish(userId, data); return data
  }

  async complete(userId: number, id: number) { return this.changeStatus(userId, id, 'COMPLETED') }
  async cancel(userId: number, id: number) { return this.changeStatus(userId, id, 'CANCELLED') }
  async remove(userId: number, id: number) {
    const order = await this.getOwned(userId, id)
    if (!['PENDING', 'PAID', 'COMPLETED'].includes(order.status as string)) throw new BadRequestException('当前订单状态不允许删除')
    await this.prisma.$transaction(async tx => {
      // 下单时已经扣减库存，删除未完成订单时必须恢复库存和销量；已完成订单只清理记录。
      const items = await tx.orderItems.findMany({ where: { orderId: order.id } })
      if (order.status !== 'COMPLETED') for (const item of items) await tx.books.update({ where: { id: item.bookId }, data: { stock: { increment: item.quantity }, salesCount: { decrement: item.quantity } } })
  ...
  async refund(userId: number, id: number, reason: string) {
    ...
    const updated = await this.prisma.$transaction(async tx => {
      ...
      await tx.refundRecords.create({ data: { orderId: order.id, refundNo: `RF${Date.now()}${Math.random().toString(36).slice(2, 7)}`, amount: order.totalAmount, reason: reason || '用户申请退款' } })
      return tx.orders.findUniqueOrThrow({ where: { id: order.id } })
    })
    const data = await this.format(updated); this.realtime.publish(userId, data); return data
  }
  ...
  async afterSale(userId: number, id: number, reason: string) {
    ...
    const updated = await this.prisma.orders.update({ where: { id: order.id }, data: { status: 'AFTER_SALE' as any, remark: reason.trim() || order.remark } })
    const data = await this.format(updated); this.realtime.publish(userId, data); return data
  }
  ...
  private async changeStatus(userId: number, id: number, next: OrderStatus) {
    ...
    const updated = await this.prisma.orders.update({ where: { id: order.id }, data: { status: next as any, ...(next === 'COMPLETED' ? { completedAt: new Date() } : {}), ...(next === 'CANCELLED' ? { cancelledAt: new Date() } : {}) } })
    const data = await this.format(updated); this.realtime.publish(userId, data); return data
  }
